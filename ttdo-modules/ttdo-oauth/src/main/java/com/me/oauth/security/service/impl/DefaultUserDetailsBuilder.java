...
//        this.ssoProperties = ssoProperties;
    ...
    public CustomUserDetails buildUserDetails(User user) {
        ...
        getUserDetailsWrapper().warp(
                details, user.getId(),
                Optional.ofNullable(getRequestTenantId()).orElse(user.getOrganizationId()),
                true);
...
//        Domain domain = domains.stream().filter(d -> d.getSsoTypeCode() != null && d.getDomainUrl().contains(finalRedirectUri)).findFirst().orElse(null);
//
//        if (domain != null) {
