...
import com.ttdo.gateway.helper.service.SignatureService;
import com.ttdo.gateway.helper.util.SignatureUtils;
...
import com.ttdo.gateway.helper.config.GatewayHelperProperties.Signature;
...
public class DefaultSignatureService implements SignatureService {
    ...
    protected boolean verify(RequestContext context, SignInfo signInfo, Map<String, Object> params) {
        // 验证签名客户端
        GatewayHelperProperties.Secret secret = signature.getSecrets().stream().filter(c -> StringUtils.equals(c.getSecretId(), signInfo.getSecretId())).findFirst().orElse(null);
        if (secret == null) {
